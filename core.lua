...
core = LibStub("AceAddon-3.0"):NewAddon(packedtitle, "AceConsole-3.0",
    "AceHook-3.0", "AceEvent-3.0", "AceSerializer-3.0")
...
function core:MAIL_SHOW(event, ...)
    ...
    if GetSubZoneText() then
        action.location = action.location .. "-" .. GetSubZoneText();
    end
...
local function GetItemListString(aMail)
    ...
    for index = 1, #aMail.Items do
        ...
        ItemList = ItemList .. " " .. 
            (anItem.Link or anItem.Name or "(???)") .. "x" .. 
            (anItem.Count or "0");
    end
...
local function GetItemListNameString(aMail)
    ...
    for index = 1, #aMail.Items do
        local anItem = aMail.Items[index];
        ItemList = ItemList .. " [" .. (anItem.Name or "(???)") .. "]x" ..
            (anItem.Count or "0");
    end
...
function core:ADDON_LOADED(event, ...)
    if not addon_initialized then
        ...
        for index, sentmail in pairs(MailBookKeeperHistory) do
            if sentmail.Version == nil then
                sentmail.Version = 1;
            end
            if sentmail.Version == 1 then
                sentmail.Version = 2;
                sentmail.Channel = "mail";
            end
            if sentmail.Version == 2 then
                sentmail.Version = 3;
                sentmail.InOut = "out";
            end
            if sentmail.Version == 3 then
                sentmail.Version = 4;
                sentmail.Location = "unknown";
            end
        ...
        if mooptions.History == nil then
            mooptions.History = modefaultoptions.History;
        end
        if mooptions.Cash == nil then
            mooptions.Cash = modefaultoptions.Cash;
        end
    end
end
...
local function FormatMoneyTostring(amount, category)
    local outstring = "";
    amount = tonumber(amount) or 0;
    if (amount >= 0) or (mooptions[category].Zero) then
        if mooptions[category].Graphics then
            outstring = GetCoinTextureString(amount + 0.0001);
        else
            outstring = tostring((amount + 0.0001) / 10000) .. "g";
        end
    end
    return outstring;
end
...
local function ProcessInboxMail(index)
    ...
    local packageIcon, stationeryIcon, sender, subject, money, CODAmount, 
          ...
          itemQuantity = GetInboxHeaderInfo(index);
...
function CheckMailRecipient(...)
    ...
    if C_FriendList.IsIgnored(rcpt) then
        ...
        for index, sentmail in pairs(hist) do
            if sentmail and sentmail.Channel == "mail" and sentmail.Recipient and sentmail.Recipient == rcpt then
                foundcnt = foundcnt + 1;
            end
        end
...
local function ShowHistory()
    ...
    --mailhistoryST.frame:RegisterEvent("OnEnter", function (rowframe, cellframe, data, cols, row, realrow, column, scrollingtable, ...)
    --    local celldata = data[realrow].cols[column];
    ...
    if MailBookKeeperHistoryAvaiable then
        ...
        local STFilter = function(self, row)
            ...
            for index, col in pairs(row.cols) do
                if string.find(strlower(col.value), strlower(self.QuickFilterRule)) > 0 then
                    return true;
                end
            end
            return false;
...
local function reportMailInfo()
    if outgoingmail.Valid then
        ...
        ChatFrame1:AddMessage(outgoingmail.Timestamp .. ":(to:" ..
            outgoingmail.Recipient .. ", " ..
            countableitems .. ", " .. outmoney .. " " ..
            outCOD .. ") '" .. outgoingmail.Subject .. "'");
...
function get_currencylist()
    ...
    local currencyname, currencyamount, texturePath, earnedThisWeek, weeklyMax;
    local totalMax, isDiscovered, currencyID;
    currencylist = {};
...
function get_factionlist()
    ...
    local factionname, factiondescription, standingID, barMin, barMax, barValue,
          atWarWith, canToggleAtWar, isHeader, isCollapsed, hasRep, isWatched,
          isChild, factionID;
    factionlist = {};
...
local OldOnTextChanged = nil
local OldOnTextChangedHooked = false;
...
function core:PLAYER_ENTERING_WORLD(event, ...)
    ...
    local instancename, instanceTypeID, difficulty, difficultyName, maxPlayers,
          playerDifficulty, isDynamicInstance, mapID, instanceGroupSize = GetInstanceInfo();
...
function show_gainedlost(partner, startgained, startlost)
    local gainedchange = goldgained - startgained;
    local lostchange = goldlost - startlost;
    local goldprofit = gainedchange - lostchange;
    if (mooptions.Cash.Enabled) and
        (mooptions.Cash.Zero or (abs(gainedchange) > 0.01) or
        (abs(lostchange) > 0.01) or (abs(goldprofit) > 0.01))
    then
        local msg = (partner and ("While at '" .. partner .. "' ")) or "";
        msg = msg .. "you " ..
                  ((goldprofit <= 0.0001 and "|cffff0000 spent ") or "|cff00ff00 gained ") ..
                  FormatMoneyTostring(abs(goldprofit), "Gold");
        msg = msg .. (((gainedchange > 0) and (lostchange > 0) and
                  (" |cffffffff(|cff00ff00+" ..
                      FormatMoneyTostring(abs(gainedchange), "Gold") ..
                      " |cffffffff/ |cffff0000-" ..
                      FormatMoneyTostring(abs(lostchange), "Gold") ..
                      "|cffffffff)")) or "");
...
local function show_gainedlost_currency(start, current, link)
    ...
    if (mooptions.Cash.Enabled and start ~= current) then
        local msg = "you " ..
                        ((profit <= 0.0001 and "|cffff0000 spent ") or
                            "|cff00ff00 gained ") .. tostring(abs(profit)) ..
                        " " .. link;
        ChatFrame1:AddMessage(msg);
    end
end
...
function get_currencies_status()
    local currencyname, currencyamount, texturePath, earnedThisWeek, weeklyMax;
    local totalMax, isDiscovered, currencyID, currencyDetails;
    local currencies = {};
...
function check_currencies(currencylist)
    ...
    for currencyID, currencyDetails in pairs(get_currencylist()) do
        startamount = 0;
        currencyamount = 0;
        if current[currencyID] then
            currencyamount = current[currencyID];
        end
        if currencylist[currencyID] then
            startamount = currencylist[currencyID];
        end
        if startamount ~= currencyamount then
            show_gainedlost_currency(startamount, currencyamount, currencyDetails.link);
        end
    end
end
...
function get_factions_status()
    local factionname, factiondescription, standingID, barMin, barMax, barValue;
    local atWarWith, canToggleAtWar, isHeader, isCollapsed, hasRep, isWatched;
    local isChild, listID, factionyDetails;
    local factions = {};
...
function check_factions(alist)
    ...
    for listID, Details in pairs(get_factionlist()) do
        ...
        if startamount ~= currentamount then
            show_gainedlost_currency(startamount, currentamount, ' reputation with ' .. Details.name);
        end
    end
end
...
function core:PLAYER_MONEY(event, ...)
    ...
    if goldchange > 0 then
        goldgained = goldgained + goldchange;
    elseif goldchange < 0 then
        goldlost = goldlost - goldchange;
    end
...
function dataobj.OnTooltipShow(tip)
    ...
    tip:AddLine(" ");
    local histlo = math.max(#MailBookKeeperHistory - 9, 1);
    local histhi = #MailBookKeeperHistory;
    for index = histlo, histhi do
        ...
        if item and item.Valid then
            ...
            if (item.COD or 0) > 0 then
                gold = FormatMoneyTostring(item.COD, "COD");
            end
            if (item.Money or 0) > 0 then
                gold = FormatMoneyTostring(item.Money, "Gold");
            end
            items = "(" .. (#item.Items or 0) .. " item" .. (((#item.Items or 0) ~= 1 and "s") or "") .. ")";
            trinfo = (item.From or "") .. " -> " .. (item.Recipient or "");
            tip:AddDoubleLine(
                trdate .. " |cffEDEDED" .. trinfo .. "|r " .. gold .. " " .. items, 
                (item.Subject or "")
            );
        end
    end
